...
export async function calculateTriggerScore(
  symbol: string,
  klines: any[]
): Promise<number> {
  const crypto = await prisma.crypto.findUnique({ where: { symbol } });
  if (!crypto) throw new Error(`Crypto not found: ${symbol}`);

  let score = crypto.triggerScore ?? 0;
  ...
  const takerBuyVolumes = klines.map((k: any) => parseFloat(k[9]));
  const tradeCounts = klines.map((k: any) => parseFloat(k[8]));
  ...
  const change7d =
    (currentPrice - closePrices[closePrices.length - 8]) /
  ...
  console.log(
    `📊 volume 7j : ${avgVol7.toFixed(2)}, précédent : ${avgVolPrev7.toFixed(
      2
    )}`
  );
  ...
  const avgTradesPrev7 =
    tradeCounts.slice(-14, -7).reduce((a, b) => a + b, 0) / 7;
  console.log(
    `🔁 Trades 7j : ${avgTrades7.toFixed(
      0
    )}, précédent : ${avgTradesPrev7.toFixed(0)}`
  );
  ...
  const stdDev = Math.sqrt(
    closePrices.reduce((acc, val) => acc + Math.pow(val - meanPrice, 2), 0) /
      closePrices.length
  );
  ...
  const stagnantDays = closePrices.filter(
    (p) => Math.abs(p - currentPrice) / currentPrice < 0.01
  ).length;
  if (stagnantDays >= 20) {
